...
public class Elf extends KaitaiStruct {
	...
	public static class EndianElf extends KaitaiStruct {
			...
			public DynamicSection dynamic() {
				...
				if (type() == Elf.PhType.DYNAMIC) {
					...
					if (_is_le) {
						...
						this.dynamic = new DynamicSection(_io__raw_dynamic, null, _root, _is_le);
					} else {
						this._raw_dynamic = io.readBytes(filesz());
						KaitaiStream _io__raw_dynamic = new ByteBufferKaitaiStream(_raw_dynamic);
						this.dynamic = new DynamicSection(_io__raw_dynamic, null, _root, _is_le);
		...
		public static class DynamicSectionEntry extends KaitaiStruct {
			...
			public String name() {
				...
				if (((_parent().isStringTableLinked()) && (tagEnum() == Elf.DynamicArrayTags.NEEDED))) {
		...
		public static class SectionHeader extends KaitaiStruct {
			...
			/**
			 * may reference a later section header, so don't try to access too early (use only lazy
			 * `instances`)
			 *
			 * @see
			 * <a href="https://refspecs.linuxfoundation.org/elf/gabi4+/ch4.sheader.html#sh_link">Source</a>
			 */
		 ...
		 * @see
		 * <a href="https://refspecs.linuxfoundation.org/elf/gabi4+/ch5.pheader.html#note_section">Source</a>
		 */
		public static class NoteSectionEntry extends KaitaiStruct {
			...
			/**
			 * Although the ELF specification seems to hint that the `note_name` field is ASCII this isn't the
			 * case for Linux binaries that have a `.gnu.build.attributes` section.
			 *
			 * @see
			 * <a href="https://fedoraproject.org/wiki/Toolchain/Watermark#Proposed_Specification_for_non-loaded_notes">Source</a>
			 */
			public byte[] name() {
				return name;
			}
	...
	/**
	 * File class: designates target machine word size (32 or 64 bits). The size of many integer fields in
	 * this format will depend on this setting.
	 */
	public Bits bits() {
		return bits;
	}
	...
	/**
	 * Specifies which OS- and ABI-related extensions will be used in this ELF file.
	 */
	public OsAbi abi() {
		return abi;
	}
	...
	/**
	 * Version of ABI targeted by this ELF file. Interpretation depends on `abi` attribute.
	 */
