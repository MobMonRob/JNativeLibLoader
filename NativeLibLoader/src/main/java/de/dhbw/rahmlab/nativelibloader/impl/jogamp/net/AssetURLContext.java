...
import de.dhbw.rahmlab.nativelibloader.impl.jogamp.os.AndroidVersion;
...
public abstract class AssetURLContext implements PiggybackURLContext {
    ...
    /**
     * The <i>optional</i> <i>asset</i> folder name with ending slash
     * <code>assets/</code>.
     * <p>
     * Note that the <i>asset</i> folder is not used on all platforms using the
     * <i>asset</i> protocol and you should not rely on it, use
     * {@link AssetURLConnection#getEntryName()}.
     * </p>
     *
     */
    ...
    /**
     * Create an <i>asset</i> URL, suitable even w/o the registered <i>asset</i>
     * URLStreamHandler.
     * <p>
     ...
     * @param cl the ClassLoader used to resolve the location, see
     * {@link #getClassLoader()}.
     * @return
    ...
    /**
     * Create an <i>asset</i> URL, suitable only with the registered
     * <i>asset</i> URLStreamHandler.
     * <p>
     * This is equivalent with:
     * <pre>
    ...
    /**
     * Returns the <i>asset</i> handler previously set via
     * {@link #registerHandler(ClassLoader)}, or null if none was set.
     */
    ...
    /**
     * Registers the generic URLStreamHandlerFactory via
     * {@link GenericURLStreamHandlerFactory#register()} and if successful sets
     * the <i>asset</i> <code>handler</code> for the given ClassLoader
     * <code>cl</code>.
     *
     ...
     * <p>
     * The ClassLoader is required to find the <i>asset</i> resource via it's
     * <code>URL findResource(String)</code> implementation.
     * </p>
     * <p>
     * It's <code>URL findResource(String)</code> implementation shall return
     * either an <i>asset</i> URL <code>asset:sub-protocol</code> or just the
     * sub-protocol URL.
     * </p>
     * <p>
     * For example, on Android, we <i>redirect</i> all <code>path</code> request
     * to <i>assets/</i><code>path</code>.
     * </p>
     */
     ...
     * {@inheritDoc}
     * <p>
     * This implementation attempts to resolve <code>path</code> in the
     * following order:
     * <ol>
     * <li> as a valid URL: <code>new URL(path)</code>, use sub-protocol if
     * <i>asset</i> URL</li>
     * <li> via ClassLoader:
     * {@link #getClassLoader()}.{@link ClassLoader#getResource(String) getResource(path)},
     * use sub-protocol if <i>asset</i> URL </li>
     * <li> as a File: <code>new File(path).toURI().toURL()</code>
     * </ol>
     * </p>
     * <p>
     * In case of using the ClassLoader (2) <b>and</b> if running on Android,
     * the {@link #assets_folder} is being prepended to <code>path</code> if
     * missing.
     * </p>
     *
     */
    ...
    public static URLConnection resolve(String path, final ClassLoader cl) throws IOException {
        ...
        try {
            ...
            if (DEBUG) {
                System.err.println("FAIL(1): " + e1.getMessage());
            }
        }
        ...
        if (null == conn) {
            ...
            try {
                ...
                if (DEBUG) {
                    System.err.println("FAIL(3): " + e.getMessage());
                }
            }
        }
    ...
    private static URLConnection open(final URL url) {
        ...
        try {
            ...
            if (DEBUG) {
                System.err.println("FAIL(2): " + ioe.getMessage());
            }
        }
        return null;
    }
