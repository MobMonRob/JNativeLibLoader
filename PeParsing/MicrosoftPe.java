...
public class MicrosoftPe extends KaitaiStruct {
    ...
    public static class ImportSection extends KaitaiStruct {
        ...
        public ImportSection(KaitaiStream _io, MicrosoftPe.Section _parent) {
            this(_io, _parent, null);
        }

        public ImportSection(KaitaiStream _io, MicrosoftPe.Section _parent, MicrosoftPe _root) {
            super(_io);
            this._parent = _parent;
            this._root = _root;
            _read();
        ...
        private byte[] importTableInstance;
        public byte[] importTableInstance() {
            if (this.importTableInstance != null)
                return this.importTableInstance;
            long _pos = this._io.pos();
            this._io.seek((_parent().pointerToRawData() + (_root.pe().optionalHdr().dataDirs().importTable().virtualAddress() - _parent().virtualAddress())));
            this.importTableInstance = this._io.readBytes(_root.pe().optionalHdr().dataDirs().importTable().size());
            this._io.seek(_pos);
            return this.importTableInstance;
        }
        private ArrayList<ImageImportDescriptor> importTable;
        private MicrosoftPe _root;
        private MicrosoftPe.Section _parent;
        public ArrayList<ImageImportDescriptor> importTable() { return importTable; }
        public MicrosoftPe _root() { return _root; }
        public MicrosoftPe.Section _parent() { return _parent; }
    }
